let eventsData = data.events.map(event => {
...
checkboxes.forEach(checkbox => {
  checkbox.addEventListener('change', function () {
    ...
    if (selectedCategory.length > 0) {
      data.events.filter(event => selectedCategory.includes(event.category))
        .forEach((event) => {
...
document.addEventListener("input", (e) => {
  ...
  if (searched.length > 0) {
    data.events.filter(event => event.name.toLowerCase().includes(searched) || event.description.toLowerCase().includes(searched))
      .forEach((event) => {
